...
def run(config):
  ...
  loaders, train_set, score_stack, labels_stack = utils.get_data_loaders(**{**config, 'batch_size': D_batch_size,
                                      'start_itr': state_dict['itr']})


  # Calculate SFL+ Ranking
  score_rank = torch.zeros((score_stack.size())).long()
  unique_labels = torch.unique(labels_stack)
  for label_ in unique_labels:
    class_indices = torch.where(labels_stack == label_)[0]
    scores = score_stack[class_indices]

    idx_score1= torch.argsort(scores, dim=0, descending=True)
    idx_score2= torch.argsort(idx_score1, dim=0)
    score_rank[class_indices] = idx_score2
  ...
  for epoch in range(state_dict['epoch'], config['num_epochs']):
    ...
    if config['pbar'] == 'mine':
      ...
      pbar = tqdm(loaders[0])
    for i, (x, y, index) in enumerate(pbar):
      ...
      metrics = train(x, y, score_rank[index])
